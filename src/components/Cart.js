...
export const generateCartItemsFrom = (cartData, productsData) => {
  const cart = cartData.map((cartItem) => {
    return {
      ...
      qty: cartItem.qty,
      ...productsData.filter((productItem) => {
        if (cartItem.productId === productItem._id) return productItem;
      })[0],
    };
  });
...
const Cart = ({ products, items = [], handleQuantity, isReadOnly=false }) => {
  ...
  return (
    <>
      ...
      {isReadOnly === true && (
        <Box className="cart" paddingBottom="1rem">
