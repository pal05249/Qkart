  ...
  return (
    <Box display="flex" flexDirection="column">
      ...
      <Stack direction="row" my="1rem">
        ...
        <Button
          variant="text"
          onClick={() => handleNewAddress({ isAddingNewAddress: false })}
        >
          Cancel
        </Button>
      </Stack>
...
const Checkout = () => {
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      //update the address list on successful post request
      setAddresses({ ...addresses, all: response.data });
  ...
  const validateRequest = (items, addresses) => {
     //Not enough balance available to checkout cart items
    ...
    if(addresses.all.length===0)
    {
      ...
      //No address selected for checkout
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    { let url=config.endpoint+"/cart/checkout"
     try{ const response=await axios.post(url,{addressId:addresses.selected},{headers:{
        Authorization:`Bearer ${token}`
      }})
      if(response.status===200)
      {
        ...
        history.push('/thanks')
        return enqueueSnackbar("Order placed successfully",{variant:'success'})
      }
    }
      catch(e)
      {
  ...
  useEffect(() => {
    if(!localStorage.getItem('token')) {
      history.push("/login")
      return enqueueSnackbar("You must be logged in to access checkout page",{variant:"error"})
    }
      ...
      <Grid container>
        <Grid item xs={12} md={9}>
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {addresses.all.length === 0 ? (
                ...
                addresses.all.map((item) => (
                  <Box
                    className={`address-item ${addresses.selected===item._id?"selected":"not-selected"}`}
                    display="flex"
                  ...
                  >
                    ...
                    <Button onClick={()=>{deleteAddress(localStorage.getItem("token"),item._id)}}>
                      <Delete />
                      Delete
                    </Button>
                  </Box>
            ...
            {newAddress.isAddingNewAddress === false &&<Button
              color="primary"
              variant="contained"
              id="add-new-btn"
              size="large"
            ...
            <Button startIcon={<CreditCard />} variant="contained" onClick={()=>{performCheckout(localStorage.getItem('token'),items,addresses)}}>
              PLACE ORDER
            </Button>
          </Box>
        </Grid>
